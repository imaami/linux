 ...
 *
 *	Copyright 2021-2024 Ian Pilcher <arequipeno@gmail.com>
 */
...
/**
 * blkdev_trig_get_bdev_file() - Get a block device file by path.
 ...
 * Return:	The block device file, or an error pointer.
 */
static struct file *blkdev_trig_get_bdev_file(const char *path, size_t len)
{
	...
	bdev_file = bdev_file_open_by_path(strim(buf), BLK_OPEN_READ, NULL,
					   NULL);
	kfree(buf);
	return bdev_file;
}
						    ...
						    size_t len)
{
	...
	bdev_file = blkdev_trig_get_bdev_file(path, len);
	if (IS_ERR(bdev_file))
					...
					const char *buf, size_t count)
{
	...
	bdev_file = blkdev_trig_get_bdev_file(buf, count);
	if (IS_ERR(bdev_file))
