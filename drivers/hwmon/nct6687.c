...
#define NCT6683_NUM_REG_VOLTAGE (sizeof(nct6683_voltage_definition) / sizeof(struct voltage_reg))
#define NCT6683_NUM_REG_TEMP 7
#define NCT6683_NUM_REG_FAN 8
#define NCT6683_NUM_REG_PWM 8

#define NCT6683_REG_MON(x) (0x100 + (x)*2)
#define NCT6683_REG_VOLTAGE(x) (0x120 + (x)*2)
...
#define NCT6683_REG_FANIN_CFG(x) (0xA00 + (x))
...
#define NCT6683_REG_FAN_CTRL_MODE(x) 0xA00
#define NCT6683_REG_FAN_PWM_COMMAND(x) 0xA01
...
static struct voltage_reg nct6683_voltage_definition[] = {
	// +12V
	{
		.reg = 0,
		.multiplier = 12,
...
static const char *const nct6683_temp_label[] = {
	"CPU",
	"System",
	"VRM MOS",
	"PCH",
...
static const char *const nct6683_fan_label[] = {
	"CPU Fan",
	"Pump Fan",
	"System Fan #1",
	"System Fan #2",
...
struct nct6683_data {
	...
	/* Temperature values */
	s32 temperature[3][NCT6683_NUM_REG_TEMP]; // 0 = current 1 = min 2 = max

	/* Fan attribute values */
	u16 rpm[3][NCT6683_NUM_REG_FAN]; // 0 = current 1 = min 2 = max
...
static void nct6683_save_fan_control(struct nct6683_data *data, int index);

static const char *nct6683_in_label(char* buf, int index)
{
	if (manual)
		sprintf(buf, "in%d", index);
	else
		strcpy(buf, nct6683_voltage_definition[index].label);

	return buf;
}

static struct attribute_group *
nct6683_create_attr_group(struct device *dev, const struct sensor_template_group *tg, int repeat)
{
	struct sensor_device_attribute_2 *a2;
...
static u16 nct6683_read(struct nct6683_data *data, u16 address)
{
...
static void nct6683_write(struct nct6683_data *data, u16 address, u16 value)
{
	u8 page = (u8)(address >> 8);
	u8 index = (u8)(address & 0xFF);
...
static void nct6683_update_temperatures(struct nct6683_data *data)
{
	...
	for (i = 0; i < NCT6683_NUM_REG_TEMP; i++)
	{
		s32 value = (char)nct6683_read(data, NCT6683_REG_MON(i));
		s32 half = (nct6683_read(data, NCT6683_REG_MON(i) + 1) >> 7) & 0x1;
		...
		pr_debug("nct6683_update_temperatures[%d]], addr=%04X, value=%d, half=%d, temperature=%d\n", i, NCT6683_REG_MON(i), value, half, temperature);
	}
}
...
static void nct6683_update_voltage(struct nct6683_data *data)
{
	...
	for (index = 0; index < NCT6683_NUM_REG_VOLTAGE; index++)
	{
		s16 reg = manual ? index : nct6683_voltage_definition[index].reg;
		s16 high = nct6683_read(data, NCT6683_REG_VOLTAGE(reg)) * 16;
		s16 low = ((u16)nct6683_read(data, NCT6683_REG_VOLTAGE(reg) + 1)) >> 4;
		s16 value = low + high;
		s16 voltage = manual ? value : value * nct6683_voltage_definition[index].multiplier;
		...
		pr_debug("nct6683_update_voltage[%d], %s, reg=%d, addr=0x%04x, value=%d, voltage=%d\n", index, nct6683_in_label(buf, index), reg, NCT6683_REG_VOLTAGE(index), value, voltage);
	}

	pr_debug("nct6683_update_voltage\n");
}

static void nct6683_update_fans(struct nct6683_data *data)
{
	int i;
	...
	for (i = 0; i < NCT6683_NUM_REG_FAN; i++)
	{
		...
		pr_debug("nct6683_update_fans[%d], rpm=%d min=%d, max=%d", i, rmp, data->rpm[1][i], data->rpm[2][i]);
	}
	...
	for (i = 0; i < NCT6683_NUM_REG_PWM; i++)
	{
		...
		pr_debug("nct6683_update_fans[%d], pwm=%d", i, data->pwm[i]);
	}
}
...
show_in_label(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (manual)
		...
		return sprintf(buf, "%s\n", nct6683_voltage_definition[sattr->index].label);
}
...
static umode_t nct6683_in_is_visible(struct kobject *kobj, struct attribute *attr, int index)
{
	pr_debug("nct6683_in_is_visible[%d], attr=0x%04X\n", index, attr->mode);
	return attr->mode;
}
...
static umode_t nct6683_fan_is_visible(struct kobject *kobj, struct attribute *attr, int index)
{
	return attr->mode;
}
...
static ssize_t show_temp_label(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sensor_device_attribute *sattr = to_sensor_dev_attr(attr);

	return sprintf(buf, "%s\n", nct6683_temp_label[sattr->index]);
}
...
static umode_t nct6683_temp_is_visible(struct kobject *kobj, struct attribute *attr, int index)
{
	return attr->mode;
}
...
static ssize_t store_pwm(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 10, &val) || val > 255 || index >= NCT6683_NUM_REG_FAN)
		return -EINVAL;

	mutex_lock(&data->update_lock);

	nct6683_save_fan_control(data, index);

	mode = nct6683_read(data, NCT6683_REG_FAN_CTRL_MODE(index));
	bitMask = (u8)(0x01 << index);

	mode = (u8)(mode | bitMask);
	nct6683_write(data, NCT6683_REG_FAN_CTRL_MODE(index), mode);

	nct6683_write(data, NCT6683_REG_FAN_PWM_COMMAND(index), NCT6683_FAN_CFG_REQ);
	msleep(50);
	nct6683_write(data, NCT6683_REG_PWM_WRITE(index), val);
	nct6683_write(data, NCT6683_REG_FAN_PWM_COMMAND(index), NCT6683_FAN_CFG_DONE);
	msleep(50);
...
static void nct6683_save_fan_control(struct nct6683_data *data, int index)
{
	...
	{
		u16 reg = nct6683_read(data, NCT6683_REG_FAN_CTRL_MODE(index));
		u16 bitMask = 0x01 << index;
		u8 pwm = nct6683_read(data, NCT6683_REG_FAN_PWM_COMMAND(index));
...
static void nct6683_restore_fan_control(struct nct6683_data *data, int index)
{
	if (data->_restoreDefaultFanControlRequired[index])
	{
		u8 mode = nct6683_read(data, NCT6683_REG_FAN_CTRL_MODE(index));
		...
		nct6683_write(data, NCT6683_REG_FAN_PWM_COMMAND(index), NCT6683_FAN_CFG_REQ);
		msleep(50);
		nct6683_write(data, NCT6683_REG_PWM_WRITE(index), data->_initialFanPwmCommand[index]);
		nct6683_write(data, NCT6683_REG_FAN_PWM_COMMAND(index), NCT6683_FAN_CFG_DONE);
		msleep(50);

		data->_restoreDefaultFanControlRequired[index] = false;

		pr_debug("nct6683_restore_fan_control[%d], addr=%04X, ctrl=%04X, _initialFanPwmCommand=%d\n", index, NCT6683_REG_FAN_PWM_COMMAND(index), NCT6683_REG_PWM_WRITE(index), data->_initialFanPwmCommand[index]);
	}
}

static umode_t nct6683_pwm_is_visible(struct kobject *kobj, struct attribute *attr, int index)
{
	return attr->mode | S_IWUSR;
}

static struct sensor_device_template *nct6683_attributes_pwm_template[] = {
	&sensor_dev_template_pwm, NULL
};
...
static inline void
nct6683_init_device(struct nct6683_data *data)
{
	u8 tmp;

	pr_debug("nct6683_init_device\n");

	/* Start hardware monitoring if needed */
	tmp = nct6683_read(data, NCT6683_HWM_CFG);
	if (!(tmp & 0x80))
	{
		pr_debug("nct6683_init_device: 0x%04x\n", tmp);
		nct6683_write(data, NCT6683_HWM_CFG, tmp | 0x80);
	}

	// enable SIO voltage
	nct6683_write(data, 0x1BB, 0x61);
	nct6683_write(data, 0x1BC, 0x62);
	nct6683_write(data, 0x1BD, 0x63);
	nct6683_write(data, 0x1BE, 0x64);
	nct6683_write(data, 0x1BF, 0x65);
}

/*
 * There are a total of 8 fan inputs.
 */
static void nct6683_setup_fans(struct nct6683_data *data)
{
	int i;

	for (i = 0; i < NCT6683_NUM_REG_FAN; i++)
	{
		u16 reg = nct6683_read(data, NCT6683_REG_FAN_CTRL_MODE(i));
		u16 bitMask = 0x01 << i;
		...
		pr_debug("nct6683_setup_fans[%d], %s - addr=%04X, ctrl=%04X, rpm=%d, _initialFanControlMode=%d\n", i, nct6683_fan_label[i], NCT6683_REG_FAN_CTRL_MODE(i), reg, rpm, data->_initialFanControlMode[i]);
	}
}

static void nct6683_setup_voltages(struct nct6683_data *data)
{
	int index;
	char buf[64];
	...
	for (index = 0; index < NCT6683_NUM_REG_VOLTAGE; index++)
	{
		...
		pr_debug("nct6683_setup_voltages[%d], %s, addr=0x%04x, value=%d, voltage=%d\n", index, nct6683_in_label(buf, index), NCT6683_REG_VOLTAGE(index), value, voltage);
	}
}

static void nct6683_setup_temperatures(struct nct6683_data *data)
{
	int i;
	...
	for (i = 0; i < NCT6683_NUM_REG_TEMP; i++)
	{
		...
		pr_debug("nct6683_setup_temperatures[%d]], addr=%04X, value=%d, half=%d, temperature=%d\n", i, NCT6683_REG_MON(i), value, half, temperature);
	}
}

static void nct6683_setup_pwm(struct nct6683_data *data)
{
	int i;

	for (i = 0; i < NCT6683_NUM_REG_PWM; i++)
	{
		data->_initialFanPwmCommand[i] = nct6683_read(data, NCT6683_REG_FAN_PWM_COMMAND(i));
		data->pwm[i] = nct6683_read(data, NCT6683_REG_PWM(i));

		pr_debug("nct6683_setup_pwm[%d], addr=%04X, pwm=%d, _initialFanPwmCommand=%d\n", i, NCT6683_REG_FAN_PWM_COMMAND(i), data->pwm[i], data->_initialFanPwmCommand[i]);
	}
}
...
static int nct6683_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < NCT6683_NUM_REG_FAN; i++)
	{
		nct6683_restore_fan_control(data, i);
	}
...
static int nct6683_probe(struct platform_device *pdev)
{
	...
	pr_debug("nct6683_probe addr=0x%04X, sioreg=0x%04X\n", data->addr, data->sioreg);
	...
	nct6683_setup_pwm(data);
	nct6683_setup_temperatures(data);
	nct6683_setup_voltages(data);

	/* Register sysfs hooks */

	group = nct6683_create_attr_group(dev, &nct6683_pwm_template_group, NCT6683_NUM_REG_FAN);
	...
	group = nct6683_create_attr_group(dev, &nct6683_in_template_group, NCT6683_NUM_REG_VOLTAGE);
	...
	group = nct6683_create_attr_group(dev, &nct6683_fan_template_group, NCT6683_NUM_REG_FAN);
	...
	group = nct6683_create_attr_group(dev, &nct6683_temp_template_group, NCT6683_NUM_REG_TEMP);
	...
	scnprintf(build, sizeof(build), "%02d/%02d/%02d", nct6683_read(data, NCT6683_REG_BUILD_MONTH), nct6683_read(data, NCT6683_REG_BUILD_DAY), nct6683_read(data, NCT6683_REG_BUILD_YEAR));

	dev_info(dev, "%s EC firmware version %d.%d build %s\n", nct6683_chip_names[data->kind], nct6683_read(data, NCT6683_REG_VERSION_HI), nct6683_read(data, NCT6683_REG_VERSION_LO), build);

	hwmon_dev = devm_hwmon_device_register_with_groups(dev, nct6683_device_names[data->kind], data, data->groups);
...
static int __init sensors_nct6683_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(pdev); i++)
	{
		...
		err = platform_device_add_data(pdev[i], &sio_data, sizeof(struct nct6683_sio_data));
		if (err)
